...
{
    struct virtualchar_dev *dev_p ;

    printk(KERN_INFO"Virtualchar open success.") ;
    
    dev_p = container_of(v_inode->i_cdev, struct virtualchar_dev, cdev) ;//反推出属于当前驱动的dev
    /*将设备结构体指针赋值给文件私有数据指针*/
...
int virtualchar_init(void)
{
    ...
    {
        result = register_chrdev_region(devno, 2, "virtualchar") ;
    }else
    {
        result = alloc_chrdev_region(&devno, 0, 2, "virtualchar") ;
    ...
    /*动态申请 两个 设备结构体的内存*/
    virtualchar_dev_p = kmalloc( 2 * sizeof(*virtualchar_dev_p), GFP_KERNEL) ;
    if( !virtualchar_dev_p)
    {
        result = -ENOMEM ;
        goto fail_malloc ;
    }
    memset(virtualchar_dev_p, 0, 2 * sizeof( *virtualchar_dev_p)) ;

    virtalchar_setup_cdev(&virtualchar_dev_p[0], 0) ;/*对globalmem 设备进行设置，该函数应该是驱动自我完善的函数*/
    virtalchar_setup_cdev(&virtualchar_dev_p[1], 1) ;

    return 0 ;
    
fail_malloc:
    unregister_chrdev_region(MKDEV(virtualchar_major, 0), 2) ;
    return result ;
}

void virtualchar_exit(void)
{
    cdev_del(&(virtualchar_dev_p[0].cdev)) ;
    cdev_del(&(virtualchar_dev_p[1].cdev)) ;
