...
#include <linux/platform_data/regs-nand.h>
#include <linux/platform_data/nand.h>
...
{	
	...
	for (i = 0; i < plat_info->nr_chips; i++) {
		...
		printk("S3C NAND Driver is using hardware ECC.ECC.size%d,nand_type=%d\n",nand->ecc.size,nand_type);
#else
		...
		//mtd_device_parse_register(s3c_mtd,NULL,NULL, partition_info, plat_info->nr_partitions);
