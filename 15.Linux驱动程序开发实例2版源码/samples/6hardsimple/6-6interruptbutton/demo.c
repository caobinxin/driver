...
static void s3c_irq_eint_mask(unsigned int irq)
{
	u32 mask;
...
DECLARE_TASKLET(test_tasklet,polling_handler, (unsigned long) &button_devices);

static irqreturn_t simplekey_interrupt(int irq, void *dummy)
{
	//printk("simplekey_interrupt\n");
	set_pin_Interrupt(0);
...
int button_init_module(void)
{
	...
	sema_init(&button_devices->sem,1);
